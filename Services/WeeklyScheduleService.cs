...
public sealed class WeeklyScheduleService
{
        ...
        string diagnosticsBackupPath)
    {
        try
        {
            ...
            var diagnostics = new WeeklyScheduleDiagnosticsDocument
            {
                ...
                Participants = participants.Select(participant =>
                {
    ...
    private WeeklyScheduleCacheDocument CreateCacheDocumentUnsafe()
    {
        return new WeeklyScheduleCacheDocument
        {
    ...
    private void PersistCache(WeeklyScheduleCacheDocument? document)
    {
        if (document is null)
        {
            return;
    ...
    private static ParsedParagraph ParseParagraph(XmlNode paragraphNode, XmlNamespaceManager namespaceManager)
    {
        ...
        foreach (var run in runs)
        {
            ...
            var isAbsent =
                HasFormatValue(run, namespaceManager, "./w:rPr/w:highlight", "red") ||
                HasFormatValue(run, namespaceManager, "./w:rPr/w:shd", "red") ||
    ...
    private sealed class ParticipantAliasMatcher
    {
        ...
        public ParticipantAliasMatcher(IReadOnlyList<ParticipantIndexEntry> participants)
        {
        ...
        private static HashSet<string> BuildGloballyUniqueSingleAliases(IReadOnlyList<ParticipantIndexEntry> participants)
        {
