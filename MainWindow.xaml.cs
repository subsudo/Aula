...
public partial class MainWindow : Window, INotifyPropertyChanged
{
    ...
    private void QuickActionButton_OnPreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        ...
        var isNavigationOnly = Keyboard.Modifiers.HasFlag(ModifierKeys.Control);
        ExecuteQuickAction(participant, actionKey, isNavigationOnly);
    }
    ...
    private void ExecuteQuickAction(ParticipantIndexEntry entry, string actionKey, bool isNavigationOnly = false)
    {
        switch (actionKey)
        {
    ...
    private double? _widthBeforeBatch;

    private void ScolaPanel_OnBatchToggleRequested(object? sender, EventArgs e)
    {
        var open = ScolaPanel.IsBatchRequested;
        ScolaBatchColumn.Width = new GridLength(open ? BatchPanelWidth : 0);
        ScolaBatchColumn.MinWidth = open ? BatchPanelWidth : 0;
        AdjustWindowWidthForBatch(open);
    }

    /// <summary>
    /// Erweitert das Fenster nach rechts, damit der Batch-Bereich Platz hat, statt
    /// die bestehenden Kacheln zu quetschen. Beim Schliessen wird die alte Breite
    /// wiederhergestellt. Im maximierten Zustand bleibt die Breite unveraendert.
    /// </summary>
    private void AdjustWindowWidthForBatch(bool open)
    {
        if (WindowState != WindowState.Normal)
        {
            return;
        ...
        if (open)
        {
            ...
            Width = Math.Min(Width + BatchPanelWidth, workArea.Width);
        ...
        {
            if (_widthBeforeBatch is double previous)
            {
                Width = previous;
            }
            else
            {
                Width = Math.Max(MinWidth, Width - BatchPanelWidth);
            }
    ...
    {
        BatchPanel.SetParticipants(ScolaPanel.TrayParticipants);
    }

    private void TogglePhotoButton_OnClick(object sender, RoutedEventArgs e)
    {
        App.Config.ShowParticipantPhoto = !App.Config.ShowParticipantPhoto;
        App.SaveConfig();
        UpdatePhotoToggleGlyph();
        DetailPanel.RefreshPhotoVisibility();
    }

    private void UpdatePhotoToggleGlyph()
    {
        // RedEye = Foto sichtbar, Hide = ausgeblendet
        TogglePhotoButton.Content = App.Config.ShowParticipantPhoto ? "" : "";
    }
    ...
    private void ScolaPanel_OnQuickActionRequested(object? sender, XHub.Scola.TrayQuickActionEventArgs e)
    {
        ...
        var entry = FindIndexEntryByPath(folderPath, e.Participant.DocumentPath);
        if (entry is null)
        {
            UpdateStatus($"Kein Akten-Eintrag für {e.Participant.FullName} gefunden.");
            return;
        }

        ExecuteQuickAction(entry, e.ActionKey, e.IsNavigationOnly);
    }

    private void ScolaPanel_OnParticipantActivated(object? sender, VerlaufsakteApp.Models.Participant participant)
    {
    ...
    private ParticipantIndexEntry? FindIndexEntryByPath(string? folderPath, string? documentPath)
    {
        if (!string.IsNullOrWhiteSpace(folderPath))
        {
            var byFolder = _indexEntries.FirstOrDefault(
                e => string.Equals(e.FolderPath, folderPath, StringComparison.OrdinalIgnoreCase));
        ...
        {
            return _indexEntries.FirstOrDefault(
                e => string.Equals(e.DocumentPath, documentPath, StringComparison.OrdinalIgnoreCase));
        }

        return null;
    }

    private const string ScolaDragFormat = "AulaScolaParticipant";
    private const string ActaDragFormat = "AulaActaEntry";
    private System.Windows.Point _actaDragStart;
    private ParticipantIndexEntry? _actaDragCandidate;

    private void CurrentList_OnPreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        // Presses auf QuickAction-Buttons sollen keinen Drag starten.
        if (FindVisualAncestor<System.Windows.Controls.Primitives.ButtonBase>(e.OriginalSource as DependencyObject) is not null)
        {
            _actaDragCandidate = null;
            return;
        }

        var item = FindVisualAncestor<ListBoxItem>(e.OriginalSource as DependencyObject);
        _actaDragCandidate = item?.DataContext as ParticipantIndexEntry;
        _actaDragStart = e.GetPosition(null);
    }

    private void CurrentList_OnPreviewMouseMove(object sender, MouseEventArgs e)
    {
        if (e.LeftButton != MouseButtonState.Pressed || _actaDragCandidate is null)
        {
            return;
        }

        var position = e.GetPosition(null);
        if (Math.Abs(position.X - _actaDragStart.X) < SystemParameters.MinimumHorizontalDragDistance &&
            Math.Abs(position.Y - _actaDragStart.Y) < SystemParameters.MinimumVerticalDragDistance)
        {
            return;
        }

        var entry = _actaDragCandidate;
        _actaDragCandidate = null;
        DragDrop.DoDragDrop(CurrentListParticipantsListBox, new DataObject(ActaDragFormat, entry), DragDropEffects.Copy);
    }
    ...
    {
        e.Effects = e.Data.GetDataPresent(ScolaDragFormat) ? DragDropEffects.Copy : DragDropEffects.None;
        e.Handled = true;
    }

    private void CurrentListParticipantsListBox_OnDrop(object sender, DragEventArgs e)
    {
        if (e.Data.GetData(ScolaDragFormat) is not VerlaufsakteApp.Models.Participant participant)
        {
            return;
        }
        ...
        if (entry is null)
        {
            UpdateStatus($"Kein Akten-Eintrag für {participant.FullName} gefunden.");
            return;
        }

        AddParticipantToWorkingList(entry, clearSearchText: false, focusSearchBox: false,
            selectInWorkingList: false, openDetails: false);
        SortWorkingListByName();
        e.Handled = true;
    }

    /// <summary>Sortiert die Arbeitsliste alphabetisch nach Anzeigename (Auto-Einordnung beim Drop).</summary>
    private void SortWorkingListByName()
    {
        var ordered = _workingList.Items
            .OrderBy(GetWorkingListDisplayName, StringComparer.CurrentCultureIgnoreCase)
            .ToList();
    ...
    private string GetWorkingListDisplayName(SavedListItem item)
    {
        return _indexEntriesByParticipantKey.TryGetValue(item.ParticipantKey, out var entry)
            ? entry.DisplayName
            : Path.GetFileName(item.ParticipantKey);
    }
