...
public class WordService
{
    ...
    public void OpenDocumentAtStructuredEntryTable(string docPath, string bookmarkName, bool bringToForeground = true)
    {
        AppLogger.Info($"Word.OpenDocumentAtStructuredEntryTable start. Doc='{docPath}', Bookmark='{bookmarkName}'");
        ...
        var openedHere = false;
        var operationSucceeded = false;
        var lifecycle = BeginWordLifecycleOperation(nameof(OpenDocumentAtStructuredEntryTable), docPath, bookmarkName);
        ...
        try
        {
            ...
            // Read-only navigation: resolve and focus the existing table only.
            // Do not call Rows.Add, clipboard access, prefill, or cell writes here.
            targetTable = ResolveStructuredEntryTableForWrite(
                doc,
                bookmarkName,
                4,
                GetStructuredEntryTableDisplayName(bookmarkName));
            FocusStructuredEntryTableAtTop(app, targetTable, bringToForeground, docPath, lifecycle);

            operationSucceeded = true;
            shouldQuitCreatedApp = false;
            AppLogger.Info($"Word.OpenDocumentAtStructuredEntryTable ok. Doc='{docPath}', Bookmark='{bookmarkName}'");
        }
        catch (COMException ex) when ((uint)ex.HResult == 0x80040154)
        {
            AppLogger.Error("Word.OpenDocumentAtStructuredEntryTable COM Class not registered", ex);
            throw new InvalidOperationException("Microsoft Word wurde nicht gefunden");
        }
        catch (COMException ex)
        {
            AppLogger.Error("Word.OpenDocumentAtStructuredEntryTable COM Fehler", ex);
            throw new InvalidOperationException($"Fehler beim Zugriff auf Word: {ex.Message}", ex);
        }
        catch (Exception ex)
        {
            LogWordLifecycle(lifecycle, $"Exception='{SanitizeForLog(ex.GetType().Name)}', Message='{SanitizeForLog(ex.Message)}'.");
            AppLogger.Error("Word.OpenDocumentAtStructuredEntryTable Fehler", ex);
            throw;
        }
        finally
        {
    ...
    private static void FocusStructuredEntryTableAtTop(
        dynamic app,
        dynamic targetTable,
        bool bringToForeground,
        string docPath,
        WordLifecycleOperationContext? context = null)
    {
        ...
        try
        {
            var rowCount = (int)targetTable.Rows.Count;
            var rowIndex = rowCount >= 2 ? 2 : 1;
            cell = targetTable.Cell(rowIndex, 1);
            range = cell.Range;
            FocusRangeAtTop(app, range, bringToForeground, docPath, context);
        }
        finally
        {
            SafeReleaseCom(range, cell);
    ...
    {
        if (bookmarkName.Contains("BI_", StringComparison.OrdinalIgnoreCase))
        {
            return "BI";
        }

        if (bookmarkName.Contains("LB_", StringComparison.OrdinalIgnoreCase))
        {
            return "LB";
        }
